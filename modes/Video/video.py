...
def video_encode_result():
    if request.method == 'POST':
        ...
        text_r = request.form
        print(text_r)
        return render_template("encode-video-result.html", message=message, result=result, file=file, video_encryption=video_encryption)
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            file.save(os.path.join(
                current_app.config['UPLOAD_VIDEO_FOLDER'], filename))
            decryption = True
            filePath = os.path.join(current_app.config['UPLOAD_VIDEO_FOLDER'],filename)
            finalFilePath = filePath.replace("\\","/")                
            print("Final Path: ",finalFilePath)
            # exit(0)
            try:
                decrpytedText = decrypt(finalFilePath)
            except Exception as e:
                print("Error: ",e)

        result = request.form
        # print(result)
        return render_template("decode-video-result.html", result=text_r, decrypytedText=decrpytedText, file=file, decryption=decryption)


# encrypt Video

def encrypt_video(f_name, input_string):
    frame_extraction(f_name)
    print(os.getcwd())
    path = str(os.getcwd()) + \
        "\modes\Video\\ffmpeg-4.3.1-2020-10-01-full_build\\bin\\ffmpeg"
    print(path)
    command = path + " -i " + f_name + " -q:a 0 -map a" + \
        str(os.getcwd())+"/tmp/audio.mp3 -y"
    os.system(command)
